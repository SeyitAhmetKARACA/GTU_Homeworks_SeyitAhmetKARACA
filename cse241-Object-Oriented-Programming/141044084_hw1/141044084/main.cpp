...
bool slave(string st,int* arr,int size,fstream&,int&,int&,int,int);
...
/* gelen parametreye gore bir deger geri donduruyor
   Null(bosluk) :0 	Register :1	constant :2	hata :-1	*/
...
*/
void MOV(int* arr,int p1i,int p2iC,int p2type);
...
/* 2.parametre tipine gore calisir (p2type), 2 ise p2iC ile dizinin p1i'nci
   ...
   yazar
   p1i : p1 index , p2iC: p2 index veya Constant		*/
void ADD(int* arr,int p1i,int p2iC,int p2type);
...
/* 2.parametre tipine gore calisir (p2type), 2 ise p2iC ile dizinin p1i'nci
   ...
   yazar
   p1i : p1 index , p2iC: p2 index veya Constant 		*/
void SUB(int* arr,int p1i,int p2iC,int p2type); // p1i : p1 index , p2iC: p2 index veya Constant
...
/*	dosya satir sayisi - kapandi */
		...
		while(!is.eof() && wc){
			...
			line = toUpper(line);
			if(line != "")
				wc = slave(line, R, numberOfRegs,is,err,numberOfHlt,lof,arg);
		}
		if(numberOfHlt == 0)
...
bool slave(string fLine,int* arr,int size,fstream& fs,int& errLine,int& numberHlt,
						int lineOfFile,int arg){
	...
	if(virgulIndex(fLine) >= 0){ /* 1 tane virgul var		*/
		...
		if(p2t == 1)
			...
			cerr<<"Error Line :"<<errLine<<" Check first parameter"<<endl;
			return false;
		}
		...
		if(p3t == 1){
			...
			cerr<<"Error Line :"<<errLine<<" Check second parameter"<<endl;
			return false;
		}
		...
		if(p2t == 1)
			...
			cerr<<"Error Line :"<<errLine<<" Check the parameters"<<endl;
			return false;
		}
	}
	
	if(p2t == 1 && ip2 > size){
		cerr<<"Error Line "<<errLine<<" : entered unknown register"<<endl;
		return false;
	}else if(p3t == 1 && ip3 > size){
		cerr<<"Error Line "<<errLine<<" : entered unknown register"<<endl;
		return false;
	}
...
void MOV(int* arr,int p1,int p2,int p2type){
	if(p2type == 1)
		arr[p2] = arr[p1];		
	else if(p2type == 2)
		arr[p1] = p2;
...
void ADD(int* arr,int p1,int p2,int p2type){
	if(p2type == 1)
...
void SUB(int* arr,int p1,int p2,int p2type){
