...
(defun hwHelper(liste liste2)
	(setq eleman (car liste))
	(if (not eleman)
		liste2
		(if (listp eleman)
			(hwHelper (append eleman (cdr liste)) liste2)
			(hwHelper (cdr liste) (cons eleman liste2))
		)

	)
)

(defun List-leveller (listex)
	(setq liste55 '())
	(setq retVal (hwHelper listex liste55))
	(reverse retVal)
)
